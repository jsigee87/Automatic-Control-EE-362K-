...
sys1 = feedback(cruise1, 1);   
[resp1,T] = step(sys1, 20);
...
%convert tf to ss form for phase portrait
b = cruise1.numerator;
a = cruise1.denominator;
[A,B,C,D] = tf2ss(b,a);

[x,y] = ode45(mysystem,[0 20], [1 1]);
plot(y(:,1),y(:,2));
% Plot the main data lines
h = plot(resp1, step1, 'b-');
%time, step2, 'b-', time, step3, 'b-');
